...
const AdminLayout = () => {
  ...
  const menuItems = [
    ...
    { label: 'Review Requests', path: '/admin/requests', icon: Trash2 },
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full">
      ...
      <div className="mt-auto p-6 border-t border-gray-100 bg-surface">
         <div className="flex items-center gap-3 px-2">
            <div className="w-10 h-10 rounded-xl bg-primary/10 overflow-hidden flex items-center justify-center flex-shrink-0">
              {user?.image ? <img src={user.image} alt="" className="w-full h-full object-cover" /> : <Users className="w-5 h-5 text-primary" />}
            </div>
  ...
  return (
    <div className="flex flex-col lg:flex-row min-h-[calc(100vh-4rem)] bg-background">
      ...
      <div className="lg:hidden flex items-center justify-between p-4 bg-surface border-b border-gray-200 sticky top-16 z-30">
      ...
      {/* Sidebar Desktop */}
      <aside className="hidden lg:flex w-64 bg-surface border-r border-gray-200 flex-col sticky top-16 h-[calc(100vh-4rem)]">
        <SidebarContent />
      </aside>
      ...
      {isMobileMenuOpen && (
        <div className="lg:hidden fixed inset-0 z-40 top-[7.5rem]">
          ...
          <aside className="fixed left-0 top-[7.5rem] bottom-0 w-72 bg-surface shadow-2xl">
            <SidebarContent />
          </aside>
        </div>
      )}
