...
const Navbar = () => {
  ...
  const protectedLinks = user ? [
    ...
    ...(user.role === Role.EXPERT || user.role === Role.ADMIN ? [{ to: "/my-requests", label: "My Delete Requests" }] : []),
  ] : [];

  const allLinks = [...navLinks, ...protectedLinks];

  return (
    <nav className="bg-white/80 backdrop-blur-md border-b sticky top-0 z-50 px-4 sm:px-6 py-4 flex items-center justify-between shadow-sm">
      <div className="flex items-center space-x-4 lg:space-x-10">
        <Link to="/" className="flex items-center group">
          <div className="bg-blue-50 text-blue-600 p-1.5 rounded-lg mr-2 shadow-inner group-hover:bg-blue-600 group-hover:text-white transition-all">
      ...
      {/* Mobile Menu Panel */}
      {isOpen && (
        <div className="absolute top-full left-0 right-0 bg-white border-b shadow-xl lg:hidden animate-in slide-in-from-top duration-300">
