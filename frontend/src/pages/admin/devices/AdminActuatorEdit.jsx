...
const AdminActuatorEdit = () => {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 py-6">
          ...
          {id === 'new' ? (
            ...
            <div className="space-y-4">
              {rules.length === 0 ? (
                ...
                rules.map((rule) => (
                  <div key={rule.id} className="p-4 border rounded hover:border-blue-300 transition-colors">
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        <p className="font-bold">Target State: <span className="text-blue-600">{rule.targetState}</span></p>
                        <p className="text-xs text-gray-500">Every {rule.intervalSeconds}s • {rule.logicalOperator} Logic • {rule.enabled ? 'Enabled' : 'Disabled'}</p>
                      </div>
                      <div className="flex gap-1">
                        <button onClick={() => handleEvaluateNow(rule.id)} className="text-[10px] bg-gray-100 px-2 py-1 rounded hover:bg-gray-200 uppercase">Run</button>
                        <button onClick={() => handleOpenRuleModal(rule)} className="p-1.5 text-blue-600 hover:bg-blue-50 rounded"><Edit2 size={14} /></button>
                        <button onClick={() => handleDeleteRule(rule.id)} className="p-1.5 text-red-600 hover:bg-red-50 rounded"><Trash2 size={14} /></button>
                      </div>
                    </div>
      ...
      {/* Rule Modal */}
      {isRuleModalOpen && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black bg-opacity-50 p-4">          <div className="bg-white rounded shadow-lg w-full max-w-xl">
            ...
            <form onSubmit={handleRuleSubmit} className="p-6 space-y-6 max-h-[80vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="flex items-center gap-2 pt-6">
                  <input type="checkbox" id="re" checked={ruleFormData.enabled} onChange={(e) => setRuleFormData({...ruleFormData, enabled: e.target.checked})} />
              ...
              <div className="space-y-4 border-t pt-4">
                <div className="flex justify-between items-center">
                  ...
                  <button type="button" onClick={handleAddCondition} className="text-blue-600 text-xs font-bold">+ Add Condition</button>
                </div>
                {ruleFormData.conditions.map((cond, index) => (
                  <div key={index} className="p-4 bg-gray-50 border rounded relative">
                    <button type="button" onClick={() => handleRemoveCondition(index)} className="absolute top-2 right-2 text-red-500"><Trash2 size={16} /></button>
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                      ...
                      {cond.type === 'SENSOR_VALUE' ? (
                        <>
                          <div>
                            <label className="block text-[10px] uppercase font-bold text-gray-400">Sensor</label>
                            <select
                        ...
                        <div>
                          <label className="block text-[10px] uppercase font-bold text-gray-400">Hours (0-23)</label>
                          <div className="flex gap-2">
                            <input type="number" value={cond.startHourInclusive} onChange={(e) => {
                              const nc = [...ruleFormData.conditions];
                              nc[index].startHourInclusive = parseInt(e.target.value);
                              setRuleFormData({...ruleFormData, conditions: nc});
                            }} className="w-1/2 p-1 border rounded text-xs" />
                            <input type="number" value={cond.endHourInclusive} onChange={(e) => {
                              const nc = [...ruleFormData.conditions];
              ...
              <div className="flex gap-3 pt-4">
                <button type="button" onClick={() => setIsRuleModalOpen(false)} className="flex-1 px-4 py-2 border rounded hover:bg-gray-50">Cancel</button>
                <button type="submit" className="flex-1 px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 font-bold">Save Rule</button>
              </div>
            </form>
          </div>
        </div>
