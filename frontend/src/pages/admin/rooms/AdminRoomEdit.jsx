...
import {Activity, ArrowLeft, Edit2, Plus, Save, Settings, Trash2} from 'lucide-react';
                ...
                <div className="md:col-span-2 space-y-8">
                    <section className="bg-white p-6 border rounded shadow-sm">
                        <div className="flex justify-between items-center mb-6">
                            ...
                            <Link to={`/admin/sensors/new?roomId=${id}`}
                                  className="text-sm bg-gray-100 px-3 py-1 rounded border hover:bg-gray-200 flex items-center gap-1">
                            ...
                            {sensors.length > 0 ? sensors.map(s => (
                                <div key={s.id} className="py-3 flex justify-between items-center">
                                    <div>
                                        ...
                                        <div className="text-xs text-gray-500 uppercase">{s.type.replace('_', ' ')} •
                                            ID: {s.id}</div>
                                    </div>
                                    <div className="flex gap-2">
                                        <Link to={`/admin/sensors/${s.id}?roomId=${id}`}
                                              className="p-1.5 text-blue-600 hover:bg-blue-50 rounded"><Edit2
                                            size={16}/></Link>
                                        <button onClick={() => handleDeleteDevice('sensor', s.id)}
                    ...
                    <section className="bg-white p-6 border rounded shadow-sm">
                        <div className="flex justify-between items-center mb-6">
                            ...
                            <Link to={`/admin/actuators/new?roomId=${id}`}
                                  className="text-sm bg-gray-100 px-3 py-1 rounded border hover:bg-gray-200 flex items-center gap-1">
                            ...
                            {actuators.length > 0 ? actuators.map(a => (
                                <div key={a.id} className="py-3 flex justify-between items-center">
                                    <div>
                                        ...
                                        <div className="text-xs text-gray-500 uppercase">{a.type.replace('_', ' ')} •
                                            ID: {a.id}</div>
                                    </div>
                                    <div className="flex gap-2">
                                        <Link to={`/admin/actuators/${a.id}?roomId=${id}`}
                                              className="p-1.5 text-blue-600 hover:bg-blue-50 rounded"><Edit2
                                            size={16}/></Link>
                                        <button onClick={() => handleDeleteDevice('actuator', a.id)}
                                                className="p-1.5 text-red-600 hover:bg-red-50 rounded"><Trash2
                                            size={16}/></button>
                                    </div>
                                </div>
