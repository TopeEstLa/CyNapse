...
import {BASE_URL, deviceApi} from '../../utils/api';
import {
  ArrowLeft,
  BarChart3,
  Cpu,
...
const DeviceDetail = () => {
    ...
    const getDeviceIcon = (type) => {
        switch (type) {
            ...
            case DeviceType.CO2_SENSOR:
                return <Wind className="w-8 h-8"/>;
            case DeviceType.PEOPLE_COUNTER:
    ...
    const renderSimpleChart = () => {
        ...
        if (numericData.length < 2) return (
            <div
                className="h-40 flex items-center justify-center text-gray-300 font-bold uppercase text-[10px] tracking-widest border border-dashed border-gray-100 rounded-3xl">
        ...
        return (
            <div className="bg-gray-50 p-6 overflow-visible relative">
                <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-4">Trend (Last 20
                    readings)</p>
    ...
    if (!device || error) return (
        <div className="max-w-4xl mx-auto px-4 py-20 text-center uppercase font-black">
            ...
            <button onClick={() => navigate('/monitoring')}
                    className="text-blue-600 flex items-center gap-2 mx-auto hover:underline">
        ...
        <main className="max-w-7xl mx-auto px-4 py-4 md:py-8 space-y-6 md:space-y-8 text-gray-900">
            <header className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 border-b pb-6">
                <div className="space-y-4 text-center md:text-left">
                    ...
                    <div className="flex flex-col md:flex-row items-center gap-3 md:gap-4">
                        ...
                        <span
                            className={`px-3 py-1 rounded-lg text-xs font-bold uppercase tracking-wider ${getStatusColor(device.status)}`}>
              {device.status}
            </span>
                    </div>
                    <p className="text-xs md:text-sm text-gray-500 font-medium flex flex-wrap justify-center md:justify-start gap-x-3 gap-y-1">
                        <span>Type: <span
                            className="text-gray-900 font-bold uppercase">{device.type.replace('_', ' ')}</span></span>
                        <span>ID: <span className="text-gray-900 font-bold">{trueId}</span></span>
                        <span>Category: <span
                            className="text-blue-600 font-bold uppercase">{isSensor ? 'Sensor' : 'Actuator'}</span></span>
                    </p>
                    {device.room && (
                        <p className="text-xs md:text-sm text-gray-500 font-medium flex items-center justify-center md:justify-start gap-2">
                            <MapPin size={14} className="text-blue-500"/>
                            Room: <Link to={`/monitoring/room/${device.room.id}`}
                                        className="text-blue-600 font-bold hover:underline">{device.room.name}</Link>
                        </p>
                    )}
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <section className="lg:col-span-2 space-y-8">
                    {!isSensor && canControl && (
                        <div
                            className="bg-white p-6 md:p-8 rounded-xl border-l-4 border-l-blue-600 shadow-sm border-t border-r border-b border-gray-200">
                            <div className="flex flex-col sm:flex-row items-center justify-between gap-6">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-gray-500 font-medium">Bypass automated control
                                            rules</p>
                                    </div>
                                </div>
                    ...
                    <div
                        className="bg-white p-6 md:p-10 rounded-xl border border-gray-200 shadow-sm relative overflow-hidden group">
                      ...
                      {getDeviceUnit(device.type)}
                        ...
                        <div
                            className="grid grid-cols-2 md:grid-cols-3 gap-4 pt-8 border-t border-gray-100 text-xs uppercase font-semibold">
                            <div>
                    ...
                    {canViewHistory && (
                        <div className="bg-white p-6 md:p-8 rounded-xl border border-gray-200 shadow-sm">
                            ...
                            <div className="overflow-x-auto">
                                <table className="w-full text-left border-collapse min-w-[500px]">
                                    ...
                                    {history.length === 0 ? (
                                        <tr>
                                            <td colSpan="3"
                                                className="py-20 text-center text-gray-400 font-semibold uppercase text-xs tracking-widest">No
                            ...
                            <span
                                className="px-2 py-0.5 bg-gray-100 rounded text-[10px] font-bold uppercase text-gray-500 border border-gray-200">
                               {i === 0 ? 'Current' : 'Log'}
                            </span>
                                                </td>
                                            </tr>
                ...
                <aside className="space-y-6">
                    <div
                        className="bg-gray-900 p-6 md:p-8 rounded-xl shadow-lg text-white space-y-6 lg:sticky lg:top-24">
                        ...
                        <div className="space-y-4">
                            <div className="p-4 rounded-lg bg-gray-800 border border-gray-700 space-y-2">
                                <p className="text-[10px] font-bold uppercase tracking-wider text-gray-500">Device
                                    Hardware ID</p>
                                <p className="text-xs font-mono text-gray-300">{isSensor ? `SN-` : `AC-`}{trueId.padStart(6, '0')}</p>
                            </div>
                            <div className="p-4 rounded-lg bg-gray-800 border border-gray-700 space-y-2">
                                <p className="text-[10px] font-bold uppercase tracking-wider text-gray-500">Security
                                    Protocol</p>
                            ...
                            {device.electricityConsumption !== undefined && device.electricityConsumption !== null && (
                                <div className="p-4 rounded-lg bg-yellow-500/10 border border-yellow-500/20 space-y-2">
                                    <p className="text-[10px] font-bold uppercase tracking-wider text-yellow-500/70">Energy
                                        Usage</p>
                                    <div className="flex items-center gap-2 text-yellow-500">
                                        <Zap className="w-3.5 h-3.5"/>
                                        <span
                                            className="text-xs font-bold uppercase tracking-wider">{device.electricityConsumption} Watts</span>
                                    </div>
                                </div>
                            )}
                        </div>
