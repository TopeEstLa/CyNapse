...
import {Activity, Droplets, Heater, Home, Lightbulb, Loader2, Search, Thermometer, Wind, Zap} from 'lucide-react';
...
const AllDevices = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const mappedSensors = Array.isArray(sensors) ? sensors.map(s => ({
                    ...s,
                    deviceCategory: 'SENSOR'
                })) : [];
                const mappedActuators = Array.isArray(actuators) ? actuators.map(a => ({
                    ...a,
    ...
    const getDeviceIcon = (type) => {
        switch (type) {
            ...
            case 'HEATER':
                return <Heater className="text-red-500"/>;
            default:
                ...
                {filteredDevices.map((device) => (
                    ...
                    >
                        <div className="absolute top-0 right-0 p-3">
                            <div
                                className={`w-3 h-3 rounded-full ${device.status === 'ONLINE' ? 'bg-green-500' : 'bg-red-500'} shadow-sm animate-pulse`}></div>
                        </div>
                        ...
                        <div
                            className="bg-gray-50 rounded-xl p-4 flex flex-col items-center justify-center border border-gray-100 group-hover:bg-blue-50/50 transition-colors">
              <span className="text-xs text-gray-400 font-bold uppercase mb-1">
                ...
                {device.deviceCategory === 'SENSOR'
                    ...
                    : device.currentState || 'UNKNOWN'
                        ...
                        <div
                            className="mt-4 flex items-center justify-between text-xs text-gray-400 font-medium italic">
